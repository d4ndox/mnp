...
/*
 * globaldefs.h declares verbose as extern.
 * The legacy mnp.c used to provide the definition.
 */
int verbose = 0;
...
)
{
    ...
    transfers = cJSON_GetObjectItem(
        result,
        "transfers"
    );

    if (transfers == NULL ||
        !cJSON_IsArray(transfers)) {
        return NULL;
    }
...
static int send_rpc_connection_alert(
    const char *workdir,
    const char *txid
)
{
    ...
    while (running) {
        ...
        *transfers = get_transfers(wallet);
        ...
        if (*transfers == NULL) {
            fprintf(
                stderr,
                "mnp: invalid wallet RPC response\n"
            );
            return -1;
        }
...
static int ensure_transaction_directory(
    const char *workdir,
    const char *txid,
    mode_t mode,
    char **path
...
static int start_amount_writer(
    const char *fifo,
    const char *amount
)
{
...
static int process_transfer(
    const cJSON *transfer,
    const char *txid,
    const char *workdir,
    const char *tx_directory,
    mode_t pipe_mode
)
{
    cJSON *address_item;
    ...
    address_item = cJSON_GetObjectItem(
        transfer,
        "address"
    );
    payment_id_item = cJSON_GetObjectItem(
        transfer,
        "payment_id"
    );
    amount_item = cJSON_GetObjectItem(
        transfer,
        "amount"
    );
    double_spend_item = cJSON_GetObjectItem(
        transfer,
        "double_spend_seen"
    );
    ...
    if (address_item == NULL ||
        ...
        fprintf(
            stderr,
            "mnp: invalid transfer in RPC response\n"
        );
        goto done;
    }
...
)
{
    ...
    if (init_wallet(
        ...
        fprintf(
            stderr,
            "mnp: cannot initialize wallet RPC request\n"
        );
        goto done;
    }
