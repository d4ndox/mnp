...
struct spend_proof_args {
    const char *txid_argument;
    char *signature;
    char *message;
};

static const struct option spend_proof_options[] = {
...
)
{
    ...
    while ((option = getopt_long(
        ...
        default:
            fprintf(
                stderr,
                "mnp spend-proof: invalid option\n"
            );
            return -1;
        }
    }
    ...
    if (optind < argc) {
        fprintf(
            stderr,
            "mnp spend-proof: unexpected argument '%s'\n",
            argv[optind]
        );
        return -1;
    }
    ...
    if (args->signature == NULL ||
        ...
        fprintf(
            stderr,
            "mnp spend-proof: --signature is required\n"
        );
        return -1;
    }
